...
const styles = {
  ...
  pill: (hovered) => ({
    fontFamily: theme.fonts.sans, fontSize: 14, fontWeight: 400,
    ...
    borderRadius: theme.radius.pill, padding: '10px 22px', cursor: 'pointer',
  ...
  return (
    <div style={styles.wrapper}>
      ...
      <div style={styles.pills}>
        {prompts.map((p, i) => (
          <button key={i} style={styles.pill(hovered === i)} onClick={() => onSend(p)} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)}>{p}</button>
        ))}
      </div>
    </div>
  )
